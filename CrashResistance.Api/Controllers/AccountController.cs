...
namespace MiniTrello.Api.Controllers
{
    public class AccountController : ApiController
    {
        ...
        [AcceptVerbs("POST")]
        [POST("register")]
        public ResultMessageModel Register([FromBody] AccountRegisterModel model)
        {
            var validacion = ValidarRegistro(model);
            if (validacion != "Exito")
            {
                return new ResultMessageModel {Message = validacion};
            }
            ...
            if (accountCreated != null)
            {
                ...
                this.SendEmail(Name,model.Email,"Congratulation "+Name+", Welcome to Crash Resistance");
                return new ResultMessageModel { Message = "Se ha creado el usuario con Exito" };

            }

            return new ResultMessageModel { Message = "Hubo un error al guardar el usuario" };
        }
       ...
       [GET("profile/{accessToken}")]
        ...
        public void SendEmail(string Name,string Email,string Message)
        {
             ...
             client.Authenticator =
                ...
                request.AddParameter("from", "Crash Resistance <postmaster@sandbox22742.mailgun.org>");
        ...
        public string ValidarRegistro(AccountRegisterModel model)
        {
            if (model.ConfirmPassword != model.Password)
                return "Las contraseñias no coinciden";
            if (CorreoExiste(model.Email))
                return "El correo ya existe";
            if (UsuarioExiste(model.UserName))
                return "El usuario ya existe";

            return "Exito";
        }

        public bool CorreoExiste(string email)
        {
            var account =
            _readOnlyRepository.First<Account>(
        ...
        public bool UsuarioExiste(string usuario)
        {
            var account =
            _readOnlyRepository.First<Account>(
                account1 => account1.UserName == usuario);
